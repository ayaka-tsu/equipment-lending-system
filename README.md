...
| ブランチを切り替えたあと `npm run typecheck` が `.next/types/validator.ts ... Cannot find module` で落ちる | 前のブランチで生成された型が `.next` に残っているため。`npm run typecheck` は実行のたびに作り直すので、もう一度実行すれば直る。                                                                |
